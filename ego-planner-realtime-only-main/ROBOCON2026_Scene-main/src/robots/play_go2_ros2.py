...
def build_clean_rviz_env():
    """剥离 snap/VSCode 注入的 GTK/Qt 环境，避免 rviz2 链接错误。"""
    env = os.environ.copy()
...
class OnnxControllerRos2(OnnxController, Node):
    ...
    def get_obs(self, model, data) -> np.ndarray:
        # Watchdog: 如果超过 0.5 秒没有收到 cmd_vel，则强制停止
        if time.time() - self.last_cmd_vel_time > 0.5:
            self.latest_cmd_vel[:] = 0.0
