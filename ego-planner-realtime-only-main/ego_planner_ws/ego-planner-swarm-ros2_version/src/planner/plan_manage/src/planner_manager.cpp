...
namespace ego_planner
{
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    ***/
    ...
    /* Hard collision post-check:
       The optimizer may report success for the current local target, but for the
       ground-robot integration we still want to reject any trajectory that samples
       inside the inflated occupancy map before it is published. */
    const double collision_check_dt = std::max(0.03, pos.getTimeSum() / 80.0);
    for (double t = 0.0; t <= pos.getTimeSum() + 1e-4; t += collision_check_dt)
    {
      if (grid_map_->getInflateOccupancy(pos.evaluateDeBoorT(t)))
      {
        cout << "post_check_collision=true t=" << t << endl;
        continous_failures_count_++;
        return false;
      }
    }
