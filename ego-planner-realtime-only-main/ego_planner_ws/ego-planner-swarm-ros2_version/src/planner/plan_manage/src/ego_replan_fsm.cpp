...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    node_->declare_parameter("fsm/use_odom_for_replan_start", false);
    node_->declare_parameter("fsm/use_odom_for_finish_check", false);
    ...
    node_->get_parameter("fsm/use_odom_for_replan_start", use_odom_for_replan_start_);
    node_->get_parameter("fsm/use_odom_for_finish_check", use_odom_for_finish_check_);
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    {
      end_pt_ = next_wp;
      detour_side_preference_ = 0;
      detour_preference_clear_count_ = 0;
  ...
  void EGOReplanFSM::waypointCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
  {
    ...
    if (!have_odom_)
    {
      RCLCPP_WARN(node_->get_logger(), "Received goal but no odom yet, ignoring.");
      return;
    }
  ...
  void EGOReplanFSM::getLocalTarget()
  {
    double t;
    auto map = planner_manager_->grid_map_;
    bool force_zero_target_vel = false;
    bool used_side_target = false;
    int selected_side_sign = 0;
    const double segment_check_step = std::max(0.05, map->getResolution() * 0.8);
    const double start_ignore_distance = 0.15;

    auto segment_hits_obstacle = [&](const Eigen::Vector3d &target) -> bool
    {
      Eigen::Vector3d delta = target - start_pt_;
      ...
      Eigen::Vector3d dir = delta / length;
      for (double travel = std::min(start_ignore_distance, length);
           travel <= length + 1e-6;
           travel += segment_check_step)
      {
        Eigen::Vector3d sample = start_pt_ + dir * std::min(travel, length);
        if (map->getInflateOccupancy(sample))
        {
          return true;
        }
      }
    ...
    auto search_side_target = [&](const Eigen::Vector3d &raw_target, double target_t, Eigen::Vector3d &adjusted_target, int &used_side_sign) -> bool
    {
      Eigen::Vector3d tangent = planner_manager_->global_data_.getVelocity(
          std::min(target_t, planner_manager_->global_data_.global_duration_));
      tangent(2) = 0.0;

      if (tangent.norm() < 1e-3)
      {
        tangent = raw_target - start_pt_;
        tangent(2) = 0.0;
      }
      if (tangent.norm() < 1e-3)
      {
        tangent = end_pt_ - start_pt_;
        tangent(2) = 0.0;
      }
      if (tangent.norm() < 1e-3)
      {
      ...
      tangent.normalize();
      Eigen::Vector3d lateral(-tangent(1), tangent(0), 0.0);

      const double retreat_options[] = {0.0, 0.2, 0.4, 0.6, 0.8};
      const double lateral_magnitudes[] = {0.8, 1.1, 1.4, 1.7, 2.0};
      const int preferred_sign = (detour_side_preference_ == 0) ? 1 : detour_side_preference_;
      const int sign_order[] = {preferred_sign, -preferred_sign};

      for (double retreat : retreat_options)
      {
        Eigen::Vector3d base = raw_target - tangent * retreat;
        base(2) = raw_target(2);

        for (int sign : sign_order)
        {
          for (double lateral_mag : lateral_magnitudes)
          {
            Eigen::Vector3d candidate = base + lateral * (sign * lateral_mag);
            candidate(2) = raw_target(2);

            if (map->getInflateOccupancy(candidate) == 0 &&
                !segment_hits_obstacle(candidate))
            {
    ...
    double safe_target_t = planner_manager_->global_data_.last_progress_time_;
    Eigen::Vector3d safe_target = start_pt_;
    for (t = planner_manager_->global_data_.last_progress_time_; t < planner_manager_->global_data_.global_duration_; t += t_step)
    {
      Eigen::Vector3d pos_t = planner_manager_->global_data_.getPosition(t);
      double dist = (pos_t - start_pt_).norm();
      bool occ = map->getInflateOccupancy(pos_t);
      ...
      if (t < planner_manager_->global_data_.last_progress_time_ + 1e-5 && dist > planning_horizen_)
      {
        ...
        for (; t < planner_manager_->global_data_.global_duration_; t += t_step)
        {
          ...
          bool occ_temp = map->getInflateOccupancy(pos_t_temp);
      ...
      bool path_blocked = (!occ) && segment_hits_obstacle(pos_t);
      ...
      if (dist >= planning_horizen_)
      {
        if (occ || path_blocked)
        {
          Eigen::Vector3d side_target;
          if (search_side_target(pos_t, t, side_target, selected_side_sign))
          {
            local_target_pt_ = side_target;
            force_zero_target_vel = true;
            used_side_target = true;
            cout << "Adjust local target sideways. raw_t=" << t
                 << " side_target=" << side_target.transpose() << endl;
          }
          else if (have_safe_target)
          {
            local_target_pt_ = safe_target;
            force_zero_target_vel = true;
            cout << "Adjust local target to nearest safe point. raw_t=" << t
                 << " safe_t=" << safe_target_t << endl;
          }
          else
          {
    ...
    {
      bool terminal_occ = map->getInflateOccupancy(end_pt_);
      bool terminal_blocked = (!terminal_occ) && segment_hits_obstacle(end_pt_);
      if (terminal_occ || terminal_blocked)
      {
        Eigen::Vector3d side_target;
        if (search_side_target(end_pt_, planner_manager_->global_data_.global_duration_, side_target, selected_side_sign))
        {
          local_target_pt_ = side_target;
          force_zero_target_vel = true;
          used_side_target = true;
          cout << "Adjust terminal local target sideways. side_target="
               << side_target.transpose() << endl;
        }
        else if (have_safe_target)
        {
          local_target_pt_ = safe_target;
          force_zero_target_vel = true;
          cout << "Adjust terminal local target to safe point. safe_t=" << safe_target_t << endl;
        }
        else
        {
    ...
    if (used_side_target)
    {
      if (detour_side_preference_ != selected_side_sign)
      {
        cout << "Update detour side preference to "
             << (selected_side_sign > 0 ? "positive" : "negative") << endl;
      }
      detour_side_preference_ = selected_side_sign;
      detour_preference_clear_count_ = 0;
    }
    else if (!force_zero_target_vel)
    {
      detour_preference_clear_count_++;
      if (detour_preference_clear_count_ >= 5 && detour_side_preference_ != 0)
      {
        cout << "Clear detour side preference after stable free-space replans." << endl;
        detour_side_preference_ = 0;
      }
    }
    else
    ...
    if (force_zero_target_vel)
    {
      local_target_vel_ = Eigen::Vector3d::Zero();
    }
    else if ((end_pt_ - local_target_pt_).norm() < (planner_manager_->pp_.max_vel_ * planner_manager_->pp_.max_vel_) / (2 * planner_manager_->pp_.max_acc_))
    {
      local_target_vel_ = Eigen::Vector3d::Zero();
    }
    else
