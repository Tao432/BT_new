...
def generate_launch_description():
    ...
    use_odom_for_replan_start = LaunchConfiguration('use_odom_for_replan_start', default=False)
    use_odom_for_finish_check = LaunchConfiguration('use_odom_for_finish_check', default=False)
    obstacles_inflation = LaunchConfiguration('obstacles_inflation', default=0.18)
    ...
    thresh_replan_time_arg = DeclareLaunchArgument('thresh_replan_time', default_value=thresh_replan_time, description='Minimum trajectory execution time before replanning')
    thresh_no_replan_meter_arg = DeclareLaunchArgument('thresh_no_replan_meter', default_value=thresh_no_replan_meter, description='Distance threshold for goal and waypoint checks')
    use_odom_for_replan_start_arg = DeclareLaunchArgument('use_odom_for_replan_start', default_value=use_odom_for_replan_start, description='Use real odom as the start of each replan')
    use_odom_for_finish_check_arg = DeclareLaunchArgument('use_odom_for_finish_check', default_value=use_odom_for_finish_check, description='Use real odom for finish checking')
    obstacles_inflation_arg = DeclareLaunchArgument('obstacles_inflation', default_value=obstacles_inflation, description='Inflation radius for obstacle point cloud')
    ...
    ego_planner_node = Node(
        ...
        parameters=[
            ...
            {'fsm/fail_safe': True},
            {'fsm/use_odom_for_replan_start': use_odom_for_replan_start},
            {'fsm/use_odom_for_finish_check': use_odom_for_finish_check},
    ...
    ld.add_action(use_odom_for_replan_start_arg)
    ld.add_action(use_odom_for_finish_check_arg)
